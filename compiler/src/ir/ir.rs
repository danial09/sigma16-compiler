...
pub struct SourceMap {
    ...
    /// Cached index of source text for line/column mappings
    source_index: Option<SourceIndex>,
    /// AST node spans by id
...
impl SourceMap {
    ...
    pub fn get_instrs_at(&self, line: usize, column: usize) -> Vec<usize> {
        ...
        let mut out = Vec::new();
        for (i, mm) in self.mappings.iter().enumerate() {
            if mm.iter().any(|m| m.source_span.contains(byte)) { out.push(i); }
        }
        out
    }
...
impl SourceIndex {
    ...
    pub fn to_line_col(&self, byte: usize) -> (usize, usize) {
        let mut line = 1;
        let mut col = 1;
        for (i, ch) in self.as_str().char_indices() {
            if i >= byte { break; }
    ...
    fn as_str(&self) -> &str { "" } // Placeholder; not needed for now
}

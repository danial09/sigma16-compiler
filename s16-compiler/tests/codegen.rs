...
#[test]
fn compile_paf_sig() {
    let source = std::fs::read_to_string("../samples/paf.sig")
        .expect("Failed to read samples/paf.sig");
...
fn compile_recursion_sig() {
    ...
    assert!(asm.contains("store"), "Recursive function should save return address");
...
#[test]
fn compile_gcd_sig() {
    let source = std::fs::read_to_string("../samples/gcd.sig")
        .expect("Failed to read samples/gcd.sig");
...
fn bubble_sort_end_to_end() {
    ...
    "#;
    let asm = compile_to_sigma16(source).expect("Compilation failed");
    assert!(asm.contains("bubble_sort"), "Should contain bubble_sort label");
    assert!(asm.contains("data   5"), "Should contain initial array data");
}
